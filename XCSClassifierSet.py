...
class XCSClassifierSet:
    ...
    def delete_from_population(self):
        avg_fitness = self.fitness_sum()/float(self.numerosity_sum())
        vote_sum = sum(cl.deletion_vote(avg_fitness) for cl in self.cls)
        choice_point = vote_sum * random.random()
        vote_sum = 0.0
        i = 0
        for cl in self.cls:
            vote_sum += cl.deletion_vote(avg_fitness)
    ...
    def remove_classifier_by_instance(self,cl):
        try:
            ...
            raise
    def instert_in_population(self,cl):
        for c in self.cls:
            if c.equals(cl):
                c.numerosity += 1
                return
